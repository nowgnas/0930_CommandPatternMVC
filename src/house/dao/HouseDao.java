...
public class HouseDao {
	...
	private HouseDao() {
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/housedata", "ssafy", "ssafy");
		} catch(Exception e) {
	...
	public List<String> getGugun(String sido) {
		String sql = "select distinct gugunName from dongcode "
				+ "where sidoName=? and gugunName is not null";
		...
		try(PreparedStatement stmt = con.prepareStatement(sql)){
			...
			while(rs.next()) {
				list.add(rs.getString(1)); //"gugunName" 하나만 얻어오기 때문에 인덱스로 표기 가능
			}
			return list;
		}catch(Exception e) {
